...
const char *audio_codecs[1] = {};
const char *video_codecs[1] = {"h264"};

MOQService::MOQService(obs_data_t *settings, obs_service_t *) : server()
{
	Update(settings);
}
...
void register_moq_service()
{
	...
	info.get_protocol = [](void *) -> const char * { return "MOQ"; };
	...
	info.apply_encoder_settings = [](void *, obs_data_t *video_settings,
		...
		MOQService::ApplyEncoderSettings(video_settings,
						 audio_settings);
	};
	...
	info.get_connect_info = [](void *priv_data,
		...
		return static_cast<MOQService *>(priv_data)->GetConnectInfo(
			(enum obs_service_connect_info)type);
	};

	obs_register_service(&info);

	obs_log(LOG_INFO, "registered moq service");
}
